...
export function ensureRtcInfo(info: Partial<RtcInfo>): RtcInfo {
  ...
  if (
    boroughGid === undefined ||
    zip === undefined ||
    isIncomeEligible === undefined ||
    evictionType === undefined ||
    bbl === undefined
  ) {
...
export function getRtcHelp({
  zip,
  boroughGid,
  evictionType,
  isIncomeEligible,
  bbl,
}: RtcInfo): { title: string; url: string } {
  ...
  return {
    title: rtc
      ? "Great news! You likely have the right to a free attorney."
